...
import { type CaseCategory, type Channel, type AiStage, CATEGORY_META } from '@/components/case-meta'
import {
  ConfidenceBar,
  CategoryBadge,
  ChannelBadge,
...
const STATUS_OPTIONS = ['Auto-Approved', 'Approved', 'Rejected', 'Pending'] as const
...
export default function OperationalDashboard() {
  ...
  const counts = useMemo(() => {
    return cases.reduce(
      ...
      { auto: 0, hitl: 0, exception: 0 } as Record<CaseCategory, number>,
    )
  }, [])
    ...
    <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8 lg:px-10">
      <header className="mb-6">
        ...
        <p className="mt-1 text-sm text-[var(--ink-muted)]">
          Operational overview of claims moving through AI and human review.
        </p>
      </header>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {(Object.keys(CATEGORY_META) as CaseCategory[]).map((category) => {
          const { label, description, icon: Icon } = CATEGORY_META[category]
          ...
          return (
            ...
            >
              <div>
                ...
                <p className="mt-2 font-mono text-3xl font-semibold text-[var(--ink)]">
                  {counts[category]}
                </p>
              ...
              <Icon
                size={20}
                className={isActive ? 'text-navy-600' : 'text-[var(--ink-muted)]'}
              />
            </button>
          )
        })}
      ...
      <div className="mt-6 flex flex-wrap items-center justify-between gap-3">
        <h2 className="text-sm font-medium text-[var(--ink-muted)]">
          {activeFilter ? CATEGORY_META[activeFilter].label : 'All'} cases
        ...
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 text-sm text-[var(--ink-muted)]">
            ...
            <select
              ...
              className="rounded-lg border border-[var(--border)] bg-[var(--surface)] px-2.5 py-1.5 text-sm text-[var(--ink)] outline-none focus:border-navy-400"
            >
              <option value="all">All</option>
              {STATUS_OPTIONS.map((status) => (
                <option key={status} value={status}>
                  {status === 'Auto-Approved' ? 'Auto' : status}
                </option>
              ))}
            </select>
          </label>
